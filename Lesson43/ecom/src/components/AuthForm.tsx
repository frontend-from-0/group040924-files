...
export function validatePassword (password: string) {
  if (password.length < 6 || password.length > 50) {
    return {success: false, message: "Password must be at least 6 and max 50 characters."}
  } else {
    return {success: true}
  }
}
...
export function AuthForm({ type, onSubmit }: AuthFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (type === "signup" && !validatedPassword.success) {
      setError(validatedPassword.message ?? 'Please enter a password');
      return;
    }
